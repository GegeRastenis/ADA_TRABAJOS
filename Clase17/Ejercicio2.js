...
let oracion = prompt("Por favor ingrese una oracion: "); 

const procesarOracion = (oracion) =>{
oracion.trim();
oracion = oracion.split(" "); 
let modificarOracion = oracion.map(palabra =>
    palabra.replace(/a/g, '@'));
let findJavaScript = oracion.indexOf('javascript'); 
console.log(findJavaScript); 
    modificarOracion = modificarOracion.join(" - "); 
return modificarOracion;
}
